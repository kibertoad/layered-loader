...
- `getMany(keys: string[], loadManyParams?: LoadManyParams = LoadParams): Promise<(T | null)[]>` - sequentially attempt to retrieve data for specified keys from all caches and data sources, in an order in which those data sources were passed to the Loader constructor. Duplicate keys in the input array are automatically deduplicated to optimize performance and prevent redundant data source calls. Note that this retrieval mode doesn't support preemptive background refresh. Note that you need to manually resolve all keys upfront for this retrieval method (e. g. by using cacheKeyFromLoadParamsResolver from the Loader). The returned array holds only the values that resolved, in no particular order - it is not positionally aligned with `keys`, and keys that resolved to nothing are simply absent. Explicitly cached `null` values *are* present, so handle them when you map over the result.
...
- The check receives the value exactly as the async cache returns it. `RedisCache` / `RedisGroupCache` with `json: true` round-trip an explicitly cached `null` faithfully, so a null entry is passed to the check as `null`. Without `json`, values are stored as raw strings and an explicit `null` cannot be represented: it is written as an empty string and comes back as `''`.
...
- `json: boolean` - if false, all passed data will be sent to Redis and returned from it as-is. If true, it will be serialized using `JSON.stringify` and deserialized, using `JSON.parse`. **Set this to true if you cache `null` values.** Without it, values are raw strings and an explicit `null` has no distinct representation: it is written as an empty string and read back as `''`, indistinguishable from a cached empty string. With `json: true` a `null` is stored as the JSON document `null` and round-trips faithfully, while a missing key still reads back as `undefined`;
