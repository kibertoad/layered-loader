...
>
export class GroupLoader<LoadedValue, LoaderParams = undefined> extends AbstractGroupCache<
  LoadedValue,
  LoaderParams
> {
  ...
  protected override async resolveManyGroupValues(
    ...
    const loadValues = await this.loadManyFromLoaders(
      cachedValues.unresolvedKeys,
      group,
      loadParams,
    )
  ...
  private async loadFromLoaders(key: string, group: string, loadParams?: LoaderParams) {
    for (let index = 0; index < this.dataSources.length; index++) {
      const resolvedValue = await this.dataSources[index]
        .getFromGroup(key, group, loadParams)
        .catch((err) => {
  ...
  private async loadManyFromLoaders(keys: string[], group: string, loadParams?: LoaderParams) {
    ...
    for (let index = 0; index < this.dataSources.length; index++) {
      lastResolvedValues = await this.dataSources[index]
        .getManyFromGroup(keys, group, loadParams)
        .catch((err) => {
    ...
    if (this.throwIfUnresolved) {
      throw new Error(
        `Failed to resolve value for some of the keys (group ${group}): ${keys.join(', ')}`,
      )
    }
